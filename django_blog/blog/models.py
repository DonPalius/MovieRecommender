...
class Strategy(models.Model):
    ...
    def recommendSVD(ratings, movies, query: int, k: int = 10):
        ...
        target_movie_index = movie_data[movie_data["movieID"] == query].index[0]
        ...
        target_genres = movie_data[movie_data["movieID"] == query]["genre"].iloc[0]
        ...
        for i, (latent_factors, genres) in enumerate(zip(movie_latent_factors, movie_data["genre"])):
            ...
            if len(set(genres).intersection(target_genres)) > 0:
                similarity_score = latent_factor_similarity * len(set(genres).intersection(target_genres))
            else:
        ...
        recommended_movies = movie_data[movie_data["movieID"].isin(recommended_movieIDs)].copy()
    ...
    def recommend_BERT(movies, query: int, similarity_matrix, k: int = 10):
        ...
        similarities = pd.Series(similarity_matrix[query_idx]).sort_values(ascending=False)
