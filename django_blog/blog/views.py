...
from .models import *
from django_blog.settings import USER_DATA
from django_blog.settings import MOVIES_DATA
from django_blog.settings import MOVIE_METADATA
from django_blog.settings import CREDITS_DATA
from django_blog.settings import RATINGS_DATA
from django_blog.settings import PLOTS_DATA
from django_blog.settings import BERT_SIM_MATRIX
import time
import json
...
def get_strategy(request, movie):
    ...
    if request.method == "GET":
        ...
        pearson_strategy = Strategy.recommend_pearson_correlation(movies, ratings,movie_id)
        recommendationDice = Strategy.recommend_Dice_strategy(movies,movie_id)
        recommendationTF_IDF = Strategy.recommend_TF_IDF(movies,plots, movie_id)
        recommendationBERT = Strategy.recommend_BERT(movies,movie_id, bert_sim_matrix)
        result = {}
        for movieID in recommendationBERT[:5]:
            movie_title =  movies[movies['movieID']==movieID]['title'].values[0]

            actors, overview = get_overview_actor(movieID, credits, movies_metadata)
